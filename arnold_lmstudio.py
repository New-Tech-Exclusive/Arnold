...
Arnold (Hebbian brain) talks to an LM Studio model over the OpenAI-compatible API.
...
DEFAULT_TEACHER_PROMPT = """You are an expert conversation teacher whose sole job is to produce a concise, machine-parseable feedback block for a downstream Hebbian brain. You MUST follow these rules exactly.
...
Be concise and objective. Rate coherence high for grammatical, clear meaning. Rate novelty high for unexpected but valid phrasing. Use reinforcement positive for useful, grammatical, factual content; negative for hallucination, nonsense, or unsafe content.
If you cannot parse the input, output coherence: 0.00, novelty: 0.00, reinforcement: -1.00, issues: unparsable input, correction: Please rewrite as one short grammatical sentence., empty token lists, and a why explaining parsing failure."""
...
_CURRICULUM_SEEDS = [
    ...
    [
        ...
        "Sleep is necessary for the brain to consolidate memories and repair cells.",
    ],
    # Phase 1
    [
...
def parse_teacher_feedback(text: str) -> TeacherFeedback | None:
    ...
    novelty = _parse_float(fields.get("novelty", ""))
...
def load_brain(args: argparse.Namespace) -> Brain:
    ...
    if saved_topo is not None:
        ...
        print(f"  Topology from saved state: vocab={saved_topo.vocab_size} "
              f"embed={saved_topo.embed_dim} bs_h={saved_topo.brainstem_hidden} "
              f"cx_h={saved_topo.cortex_hidden}")
    else:
    ...
    if not ws.exists():
        brain.brainstem.unfreeze()
        print("  Fresh brain — brainstem trainable with random weights")
    else:
        print(f"  Loaded brain state from {storage_dir}")
        print(f"  Age: {brain.developmental_age} interactions  |  "
              f"Stage: {brain.developmental_stage}")
...
def run(args: argparse.Namespace) -> None:
    ...
    brain.session_start()
    print(f"  Parameters: {brain.count_parameters():,}")
    ...
    try:
        while turn < args.turns and not shutdown_requested:
            ...
            if feedback is None:
                ...
                print(
                    f"  Teacher │ coh={feedback.coherence:.2f} nov={feedback.novelty:.2f} "
            ...
            generated_text = brain._detokenize(result.generated_tokens).strip()
            ...
            if nm:
                print(
                    f"  Brain │ novelty={result.novelty_score:.3f} "
            ...
            if turn % args.save_every == 0:
                ...
                print(f"  Saved  (age={brain.developmental_age}  "
        ...
        print(f"  Final age:        {brain.developmental_age} interactions")
...
def parse_args() -> argparse.Namespace:
    ...
    g = p.add_argument_group("Arnold brain")
    g.add_argument("--storage_dir", default="./model/pretrained",
                   help="Directory containing Arnold's saved brain state")
    ...
    g.add_argument("--brainstem_hidden", type=int, default=896)
    g.add_argument("--cortex_hidden", type=int, default=896)
