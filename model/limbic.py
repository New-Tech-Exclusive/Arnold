...
class NeuromodulatorManager:
    ...
    def update(
        ...
        p = self._params
        # Sanitize inputs — NaN from novelty/cortex must not corrupt neuromodulators
        ...
        novelty_score = float(max(min(novelty_score, 1.0), 0.0))
        ...
        # Driven by novelty and reinforcement magnitude
        ach_signal = (
            p.ach_novelty_weight * novelty_score
        ...
        ne_signal = (
            ...
            + (1.0 - p.ne_uncertainty_weight) * novelty_score
        )
...
class LimbicSystem:
    ...
    def process(
        ...
        """Full limbic pipeline: detect → neuromodulators → mood → gate."""
        ...
        neuromod = self.neuromodulator_manager.update(
            signals, novelty_score, prediction_error_magnitude,
        )
