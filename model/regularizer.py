...
The regularizer layer penalises overuse of ANY pathway, forcing diversity.
"""
...
from .types_ import TransformerRegion


class Regularizer:
    """Slow-timescale metabolic modulation of transformer pathways."""

    def __init__(self, genome: Genome) -> None:
        self._genome = genome
        self._device = get_device()
        self._params = genome.regularizer

        hidden_dim = genome.topology.transformer_hidden

        # Per-neuron usage counters for each region
        self._usage: dict[TransformerRegion, torch.Tensor] = {}
    ...
    def get_penalty(self, region: TransformerRegion) -> torch.Tensor:
    ...
    def apply_penalties(self, transformer) -> None:
        """Apply metabolic penalties to transformer W_in weights.
        ...
        for region in self._genome.topology.active_regions:
            ...
            transformer.regions[region].W_in *= penalty.unsqueeze(0)
