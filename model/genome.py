...
class GenerationParams:
    ...
    # Top-k / top-p inside brain generation
    top_k: int = 50
    top_p: float = 0.95
...
class ThalamusParams:
    ...
    novelty_parietal_bias: float = 0.4
...
class PredictiveCodingParams:
    """Each cortex region predicts next input; only error propagates."""
...
class SleepParams:
    ...
    # Stage C: REM — cortex autonomous processing
    rem_steps: int = 50
...
class Genome:
    ...
    thalamus: ThalamusParams = field(default_factory=ThalamusParams)
    predictive_coding: PredictiveCodingParams = field(default_factory=PredictiveCodingParams)
    basal_ganglia: BasalGangliaParams = field(default_factory=BasalGangliaParams)
    cerebellum: CerebellumParams = field(default_factory=CerebellumParams)
    astrocyte: AstrocyteParams = field(default_factory=AstrocyteParams)
