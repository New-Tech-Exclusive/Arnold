...
class CortexRegionModule:
    ...
    def set_gain(self, gain: float) -> None:
        """Set neuromodulatory gain (called by limbic system)."""
        self._gain = gain
...
class InterRegionWiring:
    ...
    def __init__(self, genome: Genome, rng: object | None = None) -> None:
        ...
        self._firing_order: dict[CortexRegion, int] = {}
    ...
    def record_firing_order(self, region: CortexRegion, order: int) -> None:
...
class Cortex:
    ...
    def forward(
        ...
        personality: PersonalityVector,
        routing_weights: dict[CortexRegion, float] | None = None,
        ...
        for region, blob in weights.items():
            ...
            if b.numel() == expected:
                ...
                if b.numel() == sum(old_sizes):
                    ...
                    raise RuntimeError(
                        f"Cortex weight blob for {region.value} has {b.numel()} elements, "
    ...
    def set_neuromodulatory_gain(self, arousal: float = 0.0, novelty: float = 0.0, norepinephrine: float = 0.3) -> None:
        """Neuromodulatory gain: norepinephrine drives sharpening/diffusion.

        High NE → sharpen activations (focused processing).
        Falls back to arousal+novelty if NE not set.
        """
        # Sanitize inputs
        if norepinephrine != norepinephrine: norepinephrine = 0.3
        if novelty != novelty: novelty = 0.0
        # Gentler modulation: NE=1.0, novelty=1.0 → gain=1.4 (was 1.8)
        gain = 1.0 + 0.3 * norepinephrine + 0.1 * novelty
    ...
    def get_predictions(self) -> dict[CortexRegion, torch.Tensor]:
    ...
    def set_predictions(self, predictions: dict[CortexRegion, torch.Tensor]) -> None:
    ...
    def run_autonomous(self, steps: int = 50) -> list[dict[CortexRegion, RegionActivation]]:
        """REM sleep: run cortex autonomously without external input.
