...
class Decoder:
    ...
    def sample_token(
        ...
        2. Dynamic temperature from norepinephrine (or mood fallback) + surprise.
        ...
        if surprise_score == surprise_score:  # NaN guard
            temp += float(max(min(surprise_score, 1.0), 0.0)) * nov.temperature_boost
    ...
    def generate_sequence(
        ...
        # Bootstrap initial logits from encoder's pretrained vocabulary.
        # The transformer W_out is random at birth; it adds only a tiny correction
        # that grows meaningful over thousands of training turns.
        if encoder is not None and anchor_embeddings is not None:
            mean_emb = anchor_embeddings.mean(dim=0).to(device)          # (embed_dim,)
            co_h = torch.relu(mean_emb @ encoder.cooccurrence_weights.to(device))  # (hidden_dim,)
            base_recon = co_h @ encoder.output_projection.to(device)    # (embed_dim,)
            base_logits = encoder.token_embeddings.to(device) @ base_recon  # (vocab_size,)
        ...
        if anchor_embeddings is not None and encoder is not None:
            # Project mean of input embeddings to vocab-sized anchor bias
            mean_emb = anchor_embeddings.mean(dim=0)  # (embed_dim,)
            anchor = encoder.token_embeddings @ mean_emb  # (vocab_size,)
        ...
        for step in range(max_tokens):
            ...
            token = self.sample_token(
                step_logits, mood, surprise_score, generated_so_far=tokens,
                ...
                for i_idx, pre_i_t in enumerate(trace.pre_indices.tolist()):
                    ...
                    for j_idx, post_j_t in enumerate(trace.post_indices.tolist()):
                        ...
                        if flat < int(trace.activation_strengths.numel()):
                            ...
                            if pre_i < transformer.regions[region].hidden_dim and \
                               post_j < transformer.regions[region].hidden_dim:
                                transformer.regions[region].W_hidden[pre_i, post_j] += dampened_lr * s
            else:
                transformer.regions[region].hebbian_update(trace, abs(effective_lr))
